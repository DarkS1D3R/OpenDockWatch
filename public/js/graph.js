import { healthColor, escapeHtml, badgeInnerHtml, badgeTitle } from './format.js';
  ...
  if (data.compact) {
    return `
          <div class="cy-node-box cy-node-box-compact${data.faded ? ' faded' : ''}">
            ...
            <span class="cy-node-icon" style="background:${data.icon.bg}" title="${escapeHtml(badgeTitle(data.icon))}">${badgeInnerHtml(data.icon)}</span>
